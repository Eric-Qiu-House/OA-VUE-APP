			...
			<el-container>
				<el-header style="height: auto;display: block;">
					<div class="user-info-top">
						<el-avatar :size="70" :src="user.userNameF ? user.userNameF : ''"
							:style="{ backgroundColor: user.userNameF ? '' : '#409EFF', color: '#fff' }">
							<template #default>
								<!-- 如果 avatar_ 为空，显示 fullname_ 的后两位字符 -->
								<span v-if="!user.userNameF" style="writing-mode: horizontal-tb;">
									{{ user.userName.slice(-2) }}
								</span>
							</template>
						</el-avatar>
						<!-- <el-avatar :size="70" :src="user.userNameF"></el-avatar> -->
						<h2>{{ user.userName }}</h2>
